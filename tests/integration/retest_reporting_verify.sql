...
BEGIN
  ...
  -- 1. OFFICIAL_SITE publication record reached VERIFIED with evidence.
  ...
  -- 3. Engine retest scheduling is idempotent.
  ...
  PERFORM schedule_engine_retest('SYNTH-ACME', CURRENT_DATE, 50);
  SELECT count(*) INTO v_retest_dup FROM jobs j
    WHERE j.client_id=v_cid AND j.job_type='ENGINE_OBSERVATION'
      AND j.payload_json->>'scope'='RETEST';
  IF v_retest_dup <> v_retest THEN
    RAISE EXCEPTION 'FAIL retest NOT idempotent: % -> %', v_retest, v_retest_dup; END IF;
