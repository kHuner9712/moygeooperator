...
ChatGPT、豆包、DeepSeek、Gemini、元宝、Kimi、Grok 为 `EXECUTION_READY`；千问、Perplexity 保持 `CALIBRATION_REQUIRED`。后者只允许人工登录与隐私安全的结构快照，API 的普通执行入口必须拒绝，Worker Supervisor 即使直接发现已批准任务也必须在打开浏览器前以 `PAGE_ABNORMAL + PLUGIN_CALIBRATION_REQUIRED` 失败关闭。只有插件全链路真实校准完成并通过回归测试后，才能解除调度门禁。Grok 的上线证据包括 UUID 会话 URL、独立用户/助手节点、固定回答与流式控件采样、“停止模型响应”控制、最终动作栏以及真实即时删除验证；其“删除”菜单项无二次确认，因此插件契约显式声明 `delete_requires_confirmation=False`，仍由副作用意图账本和删除后目标缺失校验保护。
