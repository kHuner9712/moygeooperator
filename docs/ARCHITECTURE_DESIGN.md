...
彻底删除租户时先校验完整客户名称并写入 DELETING，停止新任务调度和数据写入。Worker 在安全检查点停止当前任务、关闭该租户 Session 并释放租约；人工登录 Chrome 未关闭或租约仍有效时不得删除。租户目录移除成功后，按外键依赖在单一事务中删除审批、证据、资料、任务、执行事件、副作用、检查点、结果、导出、校准和 Session 元数据，最终删除 tenants 记录。失败状态保持 DELETING，允许解除文件占用后幂等重试。
