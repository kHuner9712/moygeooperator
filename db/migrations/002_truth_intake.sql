-- ============================================================================
-- MOY GEO Operator · Migration 002 · Truth intake idempotency + WF-01 core
-- Adds deterministic import keys to docs/claims and the import_truth_pack()
-- function that drives WF-01 (Truth Intake & Normalization).
-- ============================================================================

-- ---- Idempotency keys ----
ALTER TABLE truth_documents ADD COLUMN import_key text;
ALTER TABLE claims ADD COLUMN import_key text;

CREATE UNIQUE INDEX uq_truth_documents_import_key
  ON truth_documents(import_key) WHERE import_key IS NOT NULL;
CREATE UNIQUE INDEX uq_claims_import_key
  ON claims(client_id, import_key) WHERE import_key IS NOT NULL;

-- Conflict / lookup support.
CREATE INDEX idx_claims_lookup ON claims(client_id, entity_id, field_key);
CREATE INDEX idx_evidence_claim ON evidence_items(claim_id);

-- ============================================================================
-- WF-01 core: import a structured Truth Pack for an existing client.
-- Idempotent via import_key (re-running never duplicates). All claims enter as
-- DRAFT; only client-provided low-risk structured fields with a Truth document
-- are auto-VERIFIED by rule. Every claim is bound to Evidence (fail-closed:
-- a claim without a document raises CLIENT_DATA_REQUIRED). Distinct claim_text
-- for the same client+entity+field raises FACT_CONFLICT.
-- ============================================================================
