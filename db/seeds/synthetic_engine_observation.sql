...
-- P0.4 adapter contract: register an enabled adapter for each SYNTH engine so
-- record_observation() does not fail closed (it refuses to record under an
-- engine with no enabled observer adapter).
SELECT register_engine_adapter(
  (SELECT id FROM engines WHERE provider='SYNTH-LOCAL' AND product='Qwen2.5'),
  'LOCAL_OLLAMA', true) AS adapter_chat;
SELECT register_engine_adapter(
  (SELECT id FROM engines WHERE provider='SYNTH-SEARCH' AND product='web-search-mock'),
  'UI_OBSERVATION', true) AS adapter_search;
...
SELECT record_observation(
  ...
  '{"wrong_facts":0,"missing_facts":0,"competitors":[],"uncertainty":false}'::jsonb,
  'LOCAL_OLLAMA') AS obs_chat;

-- Aggregate observations into time/region/language-bound profiles.
SELECT refresh_engine_surface_profiles(
  (SELECT id FROM engines WHERE provider='SYNTH-SEARCH' AND product='web-search-mock'),
  CURRENT_DATE - 7, NULL, 'SYNTH-ACME') AS profiles_search;
SELECT refresh_engine_surface_profiles(
  (SELECT id FROM engines WHERE provider='SYNTH-LOCAL' AND product='Qwen2.5'),
  CURRENT_DATE - 7, NULL, 'SYNTH-ACME') AS profiles_chat;
